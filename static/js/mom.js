...
const MoM = (() => {
  ...
  function render(mom) {
    ...
    if (mom.summary) {
      html += `
        <div class="mom-section">
          <div class="mom-section-header"><i class="fas fa-align-left" style="color:var(--accent-light)"></i> Summary</div>
    ...
    if (mom.agenda?.length) {
      html += `
        <div class="mom-section">
          <div class="mom-section-header"><i class="fas fa-list-check" style="color:var(--sky)"></i> Agenda</div>
          <div class="agenda-list">
            ${mom.agenda.map(item => `
              <div class="agenda-item">
                <i class="fas fa-circle agenda-bullet"></i>
    ...
    if (mom.key_points?.length) {
      html += `
        <div class="mom-section">
          <div class="mom-section-header"><i class="fas fa-key" style="color:var(--violet)"></i> Key Points</div>
          ${mom.key_points.map(kp => {
    ...
    if (mom.decisions?.length) {
      html += `
        <div class="mom-section">
          <div class="mom-section-header"><i class="fas fa-circle-check" style="color:var(--emerald)"></i> Decisions</div>
          ${mom.decisions.map(d => {
            const ts  = d.timestamp || '';
            const dec = d.decision  || (typeof d === 'string' ? d : '');
            return `
              <div class="decision-item">
                ${ts ? `<span class="decision-ts">${ts}</span>` : ''}
    ...
    if (mom.action_items?.length) {
      html += `
        <div class="mom-section">
          <div class="mom-section-header"><i class="fas fa-bolt" style="color:var(--amber)"></i> Action Items</div>
            ...
            return `
              <div class="action-item">
                <div class="action-meta">
                  ${ts ? `<span class="action-ts">${ts}</span>` : ''}
